...
router.post("/create", async(req, res, next) => {
    try {
        ...
        if (error) 
            return res.status(400).send(error);
        // Need to validate body before continuing
        const task = new Task({userId: req.body.userId,title: req.body.title, category: req.body.category, content: req.body.content, dateAdded: req.body.dateAdded, completionDate: req.body.completionDate})
        await task.save();
        //task.push(task)
        return res.send(task);
    } catch (ex) {
